...
fn squeezenet_mushroom() -> ort::Result<()> {
    ...
    let session = Session::builder()?
        ...
        .commit_from_url(
            "https://parcel.pyke.io/v2/cdn/assetdelivery/ortrsv2/ex_models/squeezenet.onnx",
        )
    ...
    let input0_shape: &Vec<i64> = session.inputs[0]
        .input_type
        .tensor_dimensions()
        .expect("input0 to be a tensor type");
    let output0_shape: &Vec<i64> = session.outputs[0]
        .output_type
        .tensor_dimensions()
        .expect("output0 to be a tensor type");
